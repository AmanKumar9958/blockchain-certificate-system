    ...
    const handleVerify = async (e) => {
        ...
        try {
            ...
            if (error.response && error.response.data && error.response.data.message) {
                ...
                setVerificationResult({ verified: false, message: "Verification failed. Please check the ID or server connection." });
            }
        } finally {
            setLoading(false);
        }
        ...
        if (!verificationResult) {
            return (
                <div className="text-center text-slate-400">
                    <DocumentSearchIcon className="h-16 w-16 mx-auto mb-4 text-slate-500" />
        ...
        <div className="min-h-screen bg-slate-900 text-white flex items-center justify-center p-4">
            <div className="relative w-full max-w-md">
                ...
                <div className="relative bg-slate-800/50 backdrop-blur-lg border border-slate-700/50 rounded-2xl shadow-2xl p-8">
                    ...
                    <form onSubmit={handleVerify} className="flex gap-4 flex-col items-center sm:flex-row md:flex-row">
                        <input
                            type="text"
