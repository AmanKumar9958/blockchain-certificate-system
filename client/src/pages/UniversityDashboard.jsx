        ...
        try {
            ...
            console.error(err);
             if (err.response && err.response.data && err.response.data.message) {
                ...
                setError('Failed to issue certificate. Please check server connection.');
            }
        } finally {
            setLoading(false);
        }
                ...
                <div className="relative bg-slate-800/50 backdrop-blur-lg border border-slate-700/50 rounded-2xl shadow-2xl p-8">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            ...
                            <input id="studentName" type="text" value={studentName} onChange={(e) => setStudentName(e.target.value)} required className="mt-1 block w-full bg-slate-700/50 border border-slate-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition"/>
                        </div>
                        <div>
                            <label htmlFor="studentId" className="block text-sm font-medium text-slate-300">Student ID</label>
                            <input id="studentId" type="text" value={studentId} onChange={(e) => setStudentId(e.target.value)} required className="mt-1 block w-full bg-slate-700/50 border border-slate-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition"/>
                        </div>
                        <div>
                            <label htmlFor="courseName" className="block text-sm font-medium text-slate-300">Course Name</label>
                            <input id="courseName" type="text" value={courseName} onChange={(e) => setCourseName(e.target.value)} required className="mt-1 block w-full bg-slate-700/50 border border-slate-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition"/>
                        </div>
                        <button type="submit" disabled={loading} className="w-full flex justify-center items-center py-3 px-4 border border-transparent rounded-full shadow-lg text-sm font-bold text-white bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all transform hover:scale-105">
                            {loading ? ( <><svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Issuing Certificate...</> ) : ( 'Issue Certificate' )}
                        </button>
                    </form>
                        ...
                        <div className="mt-8 border-t border-slate-700 pt-6 text-center">
                            ...
                            {qrCodeDataUrl && (
                                <div className="mt-6 flex flex-col items-center">
                                    <h4 className="text-sm font-semibold text-slate-300 mb-2">Scannable QR Code</h4>
                                    <div className="bg-white p-3 rounded-xl shadow-md">
                                        <img src={qrCodeDataUrl} alt="Certificate QR Code" className="w-40 h-40 object-contain"/>
                                    </div>
                                    <a href={qrCodeDataUrl} download={`certificate-QR-${certificateId}.png`} className="mt-3 inline-flex items-center px-4 py-2 rounded-full bg-slate-700/60 hover:bg-slate-700 text-white text-sm font-medium transition">
                                        Download QR
                                    </a>
                                </div>
                            )}
